...
/* ------------------------------------------------------------------
   Edit this grid to customise the prompts for your own event.
   5 rows x 5 columns, row-major (matches the B / I / N / G / O header).
   Each task carries a `cat` (one of CATEGORY_META's keys) used to work
   out the player's live "AI persona." Exactly one task should have
   `free: true` — it still asks for a selfie, just with no rule attached.
------------------------------------------------------------------- */
const TASKS = [
  [
...
const STORAGE_KEY = "aiHumanBingoV1";
...
/** @type {{title: string, photos: Record<string, string>}} */
let state = { title: "", photos: {} };
let wasComplete = false;
let completedLines = new Set();
let toastTimer = null;

function cellIndex(row, col) { return row * 5 + col; }
function taskAt(idx) { return TASKS[Math.floor(idx / 5)][idx % 5]; }
...
function updatePersonaHint(counts) {
  ...
  const key = dominantCategory(counts);
  if (!key) {
    personaHint.hidden = true;
    return;
  }
...
function updateProgress(triggerEffects) {
  ...
  const offset = RING_CIRCUMFERENCE * (1 - count / TOTAL_CELLS);
  ...
  const isComplete = count >= TOTAL_CELLS;
...
async function exportSouvenir() {
  ...
  try {
    ...
    const HEADER_H = 220;
    const LETTERS_H = CELL * 0.62;
    const CAPTION_H = 66;
    const CELL_TOTAL = CELL + CAPTION_H;
    const FOOTER_H = 76;
    const H = HEADER_H + LETTERS_H + GAP + (CELL_TOTAL + GAP) * 5 + FOOTER_H;
    ...
    // grid
    const gridTop = HEADER_H + LETTERS_H + GAP;
    const photoEntries = [];
    for (let row = 0; row < 5; row++) {
      for (let col = 0; col < 5; col++) {
        const idx = cellIndex(row, col);
    ...
    ctx.fillText(`${count}/25 captured`, PAD, footerY + 5);
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `ai-human-bingo-${slug}-${stamp}.png`;
